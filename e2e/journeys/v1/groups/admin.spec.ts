/**
 * V1 Groups - Admin Tests
 *
 * Tests for the V1 Groups page (/iam/user-access/groups) with admin privileges.
 * Admin users can view, create, edit, and delete groups.
 *
 * Test Pattern:
...
import { AUTH_V1_ADMIN, SEEDED_GROUP_NAME } from '../../../utils';
...
test.describe('V1 Groups - Admin', () => {
  ...
  test('Can search for seeded group', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      // Verify the seeded group appears in results
  ...
  /**
   * Admin can see Create Group button
   */
 ...
 */
test.describe('V1 Groups - Admin CRUD Lifecycle', () => {
  test.describe.configure({ mode: 'serial' });

  const timestamp = Date.now();
  const groupName = `${TEST_PREFIX}__Lifecycle_Group_${timestamp}`;
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    console.log(`\n[V1 Groups Admin] Using prefix: ${TEST_PREFIX}`);
    console.log(`[V1 Groups Admin] Group name: ${groupName}\n`);
  });
  ...
  test('Verify group deleted', async () => {
    ...
    console.log(`\n[V1 Groups Admin] Lifecycle test completed successfully!\n`);
  });
});
