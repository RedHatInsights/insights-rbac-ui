...
**Permissions**: `rbac:principal:read` only
...
| Workspaces | ❌ Blocked | ❌ | Lacks `inventory:groups:read` |
...
**Permissions**: (none) — no RBAC or Inventory permissions
...
On stage, this may result in workspaces being visible to users who wouldn't normally have access.

For E2E tests, assume:
- Admin has workspace access (when flag is enabled)
- UserViewer and ReadOnlyUser do not have workspace access
