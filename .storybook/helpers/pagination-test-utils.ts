...
export const getLastPageNumber = (total: number, perPage: number) =>
  Math.max(1, Math.ceil(total / perPage));

export const getLastPageOffset = (total: number, perPage: number) =>
  Math.max(0, (getLastPageNumber(total, perPage) - 1) * perPage);

export const getLastCallArg = <TArg = any>(spy: { mock: { calls: unknown[][] } }, argIndex = 0): TArg => {
