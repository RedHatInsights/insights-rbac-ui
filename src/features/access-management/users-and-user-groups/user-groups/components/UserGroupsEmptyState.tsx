 ...
 */
export const UserGroupsEmptyState: React.FC<UserGroupsEmptyStateProps> = ({ titleText }) => {
  const intl = useIntl();

  return (
    <EmptyState>
      <EmptyStateHeader
        titleText={titleText || intl.formatMessage(messages.userGroupsEmptyStateTitle)}
        headingLevel="h4"
        icon={<EmptyStateIcon icon={SearchIcon} />}
      />
      <EmptyStateBody>{intl.formatMessage(messages.userGroupsEmptyStateSubtitle)}</EmptyStateBody>
    </EmptyState>
  );
};
