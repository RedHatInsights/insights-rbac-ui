...
export const DrilldownMenuInteraction: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(async () => {
      await expect(canvas.getByText('Menu Item 1')).toBeInTheDocument();
    });

    await expect(canvas.getByText('Menu Item 2')).toBeInTheDocument();
  },
};
...
export const ExternalLinkAction: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const manageNotifications = canvas.getByText('Manage notifications');
