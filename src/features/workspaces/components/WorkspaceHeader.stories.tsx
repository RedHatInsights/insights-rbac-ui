...
export const Default: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(titleElements.length).toBe(2); // Once in title, once in breadcrumb

    // Verify description is displayed
    await expect(canvas.getByText('Main production workspace for critical services and applications')).toBeInTheDocument();

    // Verify action menu is present (kebab menu button)
    const actionButton = canvas.getByRole('button', { name: /workspace actions|actions|menu/i });
...
export const WithHierarchy: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(webServicesElements.length).toBe(2); // Once in title, once in breadcrumb

    await expect(canvas.getByText('Frontend web applications and API services')).toBeInTheDocument();
...
export const LoadingState: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const skeletons = document.querySelectorAll('.pf-v5-c-skeleton');
...
export const NoAssets: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(emptyWorkspaceElements.length).toBe(2);

    await expect(canvas.getByText('A workspace with no child assets or workspaces')).toBeInTheDocument();
...
export const LongContent: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(longNameElements.length).toBe(2);

    await expect(canvas.getByText(/This is a very long description/)).toBeInTheDocument();

    // Verify deep hierarchy breadcrumb
    await expect(canvas.getByText('Root')).toBeInTheDocument();
    await expect(canvas.getByText('Level 1 Parent')).toBeInTheDocument();
    await expect(canvas.getByText('Level 2 Parent')).toBeInTheDocument();
  },
};
...
export const ActionsInteraction: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const actionsButton = canvas.getByRole('button', { name: /workspace actions|actions|menu/i });
