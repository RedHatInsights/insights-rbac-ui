...
export const ModalOpen: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    await expect(body.getByText('Move "Web Services"')).toBeInTheDocument();

    // Verify warning text about permission changes
    await expect(body.getByText(/Moving a workspace may change who is able to access it and their permissions/)).toBeInTheDocument();

    // Verify parent workspace section header
    await expect(body.getByText('Parent workspace')).toBeInTheDocument();

    // Verify action buttons are present
    await expect(body.getByText('Submit')).toBeInTheDocument();
    await expect(body.getByText('Cancel')).toBeInTheDocument();

    // Submit should be enabled since a workspace is pre-selected
    const submitButton = body.getByText('Submit');
    await expect(submitButton).toBeEnabled();

    // Test the Cancel button and onClose action
    const cancelButton = body.getByText('Cancel');
  ...
  play: async ({ canvasElement, args }) => {
    ...
    await waitFor(
      async () => {
        ...
        expect(workspaceOptions.length).toBeGreaterThan(0);
      },
      { timeout: 5000 },
    );
    ...
    await waitFor(
      async () => {
        const resultMessage = canvas.getByTestId('submission-result');
...
export const NoWorkspaceToMove: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // Modal should not appear when no workspace is provided
    const body = within(document.body);
    await expect(body.queryByRole('dialog')).not.toBeInTheDocument();
  },
};
