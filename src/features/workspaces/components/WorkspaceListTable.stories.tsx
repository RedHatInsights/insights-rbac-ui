...
export const NoPermissions: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => {
      const editButton = canvas.getByText('Edit workspace');
      const deleteButton = canvas.getByText('Delete workspace');
      const moveButton = canvas.getByText('Move workspace');
      ...
      expect(moveButton.closest('button')).toHaveAttribute('disabled');
    });
  },
};
...
export const RestrictedPermissions: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(moveButton.closest('button')).not.toHaveAttribute('disabled');
    ...
    const editButton2 = canvas.getByText('Edit workspace');
    const deleteButton2 = canvas.getByText('Delete workspace');
    const moveButton2 = canvas.getByText('Move workspace');

    // Should be disabled for workspace '2' (no permission for this workspace)
    expect(editButton2.closest('button')).toHaveAttribute('disabled');
    expect(deleteButton2.closest('button')).toHaveAttribute('disabled');
    expect(moveButton2.closest('button')).toHaveAttribute('disabled');
  },
};
