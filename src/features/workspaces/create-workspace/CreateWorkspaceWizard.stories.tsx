...
export const Default: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const openButton = canvas.getByTestId('open-wizard-button');
    ...
    const workspaceDetailsElements = body.getAllByText('Workspace details');
    ...
    const nameField = body.getByRole('textbox', { name: /workspace name/i });
    await expect(nameField).toBeInTheDocument();

    const descriptionField = body.getByRole('textbox', { name: /workspace description/i });
    ...
    expect(nextButton || cancelButton).toBeTruthy();
  },
};
...
export const LoadingWorkspaces: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(loadingElements.length > 0 || skeletons.length > 0).toBe(true);
  },
};
