...
export const Default: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText('User group')).toBeInTheDocument();
    ...
    await expect(canvas.getByText('Platform Administrators')).toBeInTheDocument();
    await expect(canvas.getByText('Development Team')).toBeInTheDocument();
    await expect(canvas.getByText('QA Engineers')).toBeInTheDocument();
...
export const EmptyState: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(
      async () => {
        // Look for the actual empty state message from the component
        await expect(canvas.getByText('No user group found')).toBeInTheDocument();
      },
      { timeout: 3000 },
    );
  },
