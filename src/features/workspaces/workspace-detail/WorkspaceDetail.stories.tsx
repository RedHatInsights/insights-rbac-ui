...
const meta: Meta<typeof WorkspaceDetail> = {
  ...
  tags: ['autodocs', 'workspaces', 'workspace-detail'],
...
export const Default: Story = {
  parameters: {
    ...
    docs: {
      description: {
        story: 'Default view showing the Assets tab with feature flags disabled for role assignments.',
      },
    },
  },
  play: async ({ canvasElement }) => {
    ...
    // Container-specific: Verify hierarchy breadcrumb is built correctly from URL params
    await expect(canvas.getByText('Workspace hierarchy:')).toBeInTheDocument();
...
export const WithRolesEnabled: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText('Role assignments')).toBeInTheDocument();
    await expect(canvas.getByText('Assets')).toBeInTheDocument();
...
export const TabSwitching: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const assetsTab = canvas.getByText('Assets');
    const rolesTab = canvas.getByText('Role assignments');
...
export const RootWorkspace: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText('Production Environment')).toBeInTheDocument();
    // Should not show any parent workspace since this is root
    await expect(canvas.queryByText('Web Services')).not.toBeInTheDocument();
  },
};
