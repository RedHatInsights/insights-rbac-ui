...
export const Default: Story = {
  play: async ({ canvasElement }) => {
    ...
    const toggleButton = canvas.getByRole('button', { name: /details/i });
    await expect(toggleButton).toBeInTheDocument();

    // Verify initial expanded state (default is true)
    const content = canvas.getByLabelText('About default groups - detailed explanation');
    await expect(content).toBeVisible();

    // Check for the view groups link
    const viewGroupsLink = canvas.getByRole('button', { name: /view your default groups/i });
