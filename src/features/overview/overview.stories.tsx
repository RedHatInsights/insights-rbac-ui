...
export const Default: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByLabelText('Get started card')).toBeInTheDocument();
    await expect(canvas.getByLabelText('Supporting features list')).toBeInTheDocument();
    await expect(canvas.getByLabelText('Recommended content table')).toBeInTheDocument();

    // Test that the main page structure is rendered
    await expect(canvas.getByRole('heading', { level: 1 })).toBeInTheDocument();

    // Verify no workspace alert shown (not eligible)
    expect(canvas.queryByText('You are qualified to opt into the workspace user access model for your organization.')).not.toBeInTheDocument();
  },
};
...
export const WithWorkspaceAlert: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText('You are qualified to opt into the workspace user access model for your organization.')).toBeInTheDocument();
  },
};
