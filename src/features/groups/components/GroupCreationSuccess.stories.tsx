...
export const Default: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    const exitButton = canvas.getByRole('button', { name: 'Exit' });
    await userEvent.click(exitButton);
    await expect(args.onClose).toHaveBeenCalled();

    // Test secondary create another button
    const createAnotherButton = canvas.getByRole('button', { name: 'Create another group' });
...
export const WithoutCreateAnother: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    const closeButton = canvas.getByRole('button', { name: 'Exit' });
