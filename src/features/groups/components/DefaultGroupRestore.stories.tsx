...
export const InteractionTest: Story = {
  play: async ({ canvasElement, args }) => {
    ...
    const restoreButton = canvas.getByRole('button', { name: /restore to default/i });
    expect(restoreButton).toBeInTheDocument();

    // Test clicking the restore button
    await userEvent.click(restoreButton);
    expect(args.onRestore).toHaveBeenCalled();

    // Find and test the info icon
    const infoIcon = canvas.getByRole('button', { name: /more information/i });
    ...
    expect(popoverContent).toHaveLength(2);
    ...
    await userEvent.click(restoreButton);
    expect(args.onRestore).toHaveBeenCalledTimes(2);
  },
};
