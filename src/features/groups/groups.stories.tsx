...
const createMockStore = (
  ...
  systemGroup = mockSystemGroup,
  isLoading = false,
  filters = { name: '' },
  pagination = { limit: 20, offset: 0, count: mockGroups.length },
) => ({
  groupReducer: {
    groups: {
      data: groups,
  ...
  // Other reducers
  userReducer: { selectedUser: {}, isUserDataLoading: false },
  workspacesReducer: { isLoading: false, workspaces: [], error: '' },
  policyReducer: { policies: { data: [], meta: { count: 0 } }, policy: {}, isLoading: false },
  roleReducer: { isLoading: false, roles: { data: [], meta: { count: 0 } } },
  accessReducer: { isLoading: false, access: { data: [], meta: { count: 0 } } },
  permissionReducer: { isLoading: false, options: {}, permission: { data: [] } },
  inventoryReducer: { isLoading: false, resourceTypes: {}, total: 0 },
  costReducer: { isLoading: false, resourceTypes: { data: [] }, resources: {} },
  serviceAccountReducer: { offset: 0, limit: 10, isLoading: false, serviceAccounts: [] },
  errorReducer: {},
  notifications: [],
});
...
export const Loading: Story = {
  parameters: {
    store: createMockStore([], undefined, undefined, true), // isLoading = true
    chrome: { environment: 'stage' },
    permissions: { orgAdmin: true, userAccessAdministrator: true },
    msw: {
      handlers: [
...
export const DefaultGroupsBehavior: Story = {
  parameters: {
    store: createMockStore(mockGroups, mockAdminGroup, mockSystemGroup),
    chrome: { environment: 'stage' },
    permissions: { orgAdmin: true, userAccessAdministrator: false },
  },
  play: async ({ canvasElement }) => {
