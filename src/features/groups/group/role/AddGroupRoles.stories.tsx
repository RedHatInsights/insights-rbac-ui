...
- **[Loading](?path=/story/features-groups-group-role-addgrouproles--loading)**: Modal in loading state
  ...
  play: async ({ canvasElement }) => {
    ...
    const submitButton = within(modal).getByRole('button', { name: /add to group/i });
...
export const WithRoles: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(
      () => {
        expect(within(modal).getByText('Content Manager')).toBeInTheDocument();
        expect(within(modal).getByText('Auditor')).toBeInTheDocument();
      },
      { timeout: 5000 },
    );
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    await waitFor(
      async () => {
        ...
        expect(modalContent.getByText('Content Manager')).toBeInTheDocument();
      },
      { timeout: 5000 },
    );
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    await waitFor(
      async () => {
        ...
        expect(modalContent.getByText('Content Manager')).toBeInTheDocument();
        expect(modalContent.getByText('Developer')).toBeInTheDocument();
...
export const AddRolesToGroupAPITest = {
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    const addButton = modalContent.getByRole('button', { name: /add to group/i });
