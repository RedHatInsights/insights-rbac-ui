...
export interface UseGroupMembersReturn {
  ...
  // Remove modal state (from useConfirmItemsModal hook)
  removeModalState: {
    isOpen: boolean;
    itemsToRemove: Member[];
    title: string;
...
export const useGroupMembers = (options: UseGroupMembersOptions = {}): UseGroupMembersReturn => {
  ...
  // Use the generic confirmation modal hook for member removal
  const { openRemoveModal: handleRemoveMembers, removeModalState } = useConfirmItemsModal<Member>({
    onConfirm: async (membersToRemove) => {
      if (!groupId) return;
    ...
    onSuccess: () => {
      ...
      // Reset offset to 0 after removal
