...
export const EditGroupRoute: Story = {
  parameters: {
    ...
    routerInitialEntries: ['/groups/group-123/members/edit-group'],
  },
  play: async ({ canvasElement }) => {
    await delay(300);
    const canvas = within(canvasElement);

    // Wait for main table content to load first
    expect(await canvas.findByText('alice.johnson')).toBeInTheDocument();

    // Check the edit group modal is rendered
    ...
    expect(within(contextData).getByText(/Group UUID: group-123/)).toBeInTheDocument();
    expect(within(contextData).getByText(/Group Name: Test Group/)).toBeInTheDocument();
    expect(within(contextData).getByText(/Admin Default: false/)).toBeInTheDocument();
    expect(within(contextData).getByText(/Platform Default: false/)).toBeInTheDocument();
    expect(within(contextData).getByText(/Cancel Route:.*\/groups\/detail\/group-123\/members/)).toBeInTheDocument();
    expect(within(contextData).getByText(/Post Method: function/)).toBeInTheDocument();

    // Test calling the postMethod to verify it works
    ...
    // Verify that the postMethod triggers a group API call (not members - that should be preserved)
    await delay(100);
...
export const RemoveGroupRoute: Story = {
  parameters: {
    ...
    routerInitialEntries: ['/groups/group-123/members/remove-group'],
  },
  play: async ({ canvasElement }) => {
    await delay(300);
    const canvas = within(canvasElement);
    ...
    // Check the remove group modal is rendered
    ...
    // Verify context data is passed correctly
    const contextData = await canvas.findByTestId('context-data');
    expect(within(contextData).getByText(/Groups UUID: group-123/)).toBeInTheDocument();
    expect(within(contextData).getByText(/Groups Name: Test Group/)).toBeInTheDocument();
    expect(within(contextData).getByText(/Cancel Route:.*\/groups\/detail\/group-123\/members/)).toBeInTheDocument();
    expect(within(contextData).getByText(/Submit Route:.*\/groups/)).toBeInTheDocument();
    expect(within(contextData).getByText(/Post Method: function/)).toBeInTheDocument();
    ...
    // Verify that the postMethod triggers a new API call (groups list refresh)
    await delay(100);
    expect(getGroupsSpy).toHaveBeenCalled();
  },
};

// Test routing functionality - Add Members route
export const AddMembersRoute: Story = {
  parameters: {
    msw: createMockHandlers(),
    permissions: {
      orgAdmin: true,
    ...
    routerInitialEntries: ['/groups/group-123/members/add-members'],
  },
  play: async ({ canvasElement }) => {
    await delay(300);
    const canvas = within(canvasElement);
    ...
    // Check the add members modal is rendered
    expect(await canvas.findByTestId('add-members-route')).toBeInTheDocument();
    expect(await canvas.findByText('Add Members Modal')).toBeInTheDocument();

    // Verify context data is passed correctly
    const contextData = await canvas.findByTestId('context-data');
    expect(within(contextData).getByText(/Cancel Route:.*\/groups\/detail\/group-123\/members/)).toBeInTheDocument();
    expect(within(contextData).getByText(/Fetch Data: function/)).toBeInTheDocument();

    // Test calling the fetchData function to verify it works
    const fetchDataButton = await canvas.findByTestId('call-fetch-data');
    expect(fetchDataButton).toBeInTheDocument();

    // Clear the API spy before testing
    getMembersSpy.mockClear();

    // Click the button to trigger fetchData
    await userEvent.click(fetchDataButton);

    // Verify that the fetchData triggers a new API call (member list refresh)
    await delay(100);
