...
import AddGroupWizard from './add-group-wizard';
...
const GroupsPage: React.FC<{ postMethod?: (data: any) => void }> = ({ postMethod = () => {} }) => {
...
const AddGroupWizardWithRouter: React.FC<{ postMethod?: (data: any) => void; enableWorkspaces?: boolean; enableServiceAccounts?: boolean }> = ({
  postMethod = () => {},
  ...
  return (
    <MemoryRouter initialEntries={['/groups']}>
      <Routes>
        <Route path="/groups" element={<GroupsPage postMethod={postMethod} />} />
...
const meta: Meta<typeof AddGroupWizardWithRouter> = {
  title: 'Features/Groups/AddGroup/AddGroupWizard',
...
export const FullWizardFlow: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    // Step 2 data: Verify role selection is reflected (if checkbox selection worked)
    const hasRoleSection = modalContent.queryByText('Roles') || modalContent.queryByText('Selected roles') || modalContent.queryByText('Viewer'); // First role in mock data
    // Don't require this since async table loading may not complete selection

    // Step 3 data: Verify user selection is reflected (if checkbox selection worked)
    const hasUserSection =
      modalContent.queryByText('Users') ||
      modalContent.queryByText('Members') ||
      modalContent.queryByText('Selected members') ||
      modalContent.queryByText('alice.doe'); // First user in mock data
    // Don't require this since async table loading may not complete selection
