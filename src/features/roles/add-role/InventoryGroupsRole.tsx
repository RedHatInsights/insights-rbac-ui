...
type Action =
  | { type: 'toggle'; key: string; isOpen: boolean; filterValue?: string; page?: number }
    ...
    case 'toggle': {
      ...
      return {
        ...
        [action.key]: {
          ...
          page: action.page ?? prevState?.page ?? 1,
        },
      };
    }
    case 'select': {
...
const InventoryGroupsRole: React.FC<InventoryGroupsRoleProps> = (props) => {
  ...
  const onSelect = (_event: unknown, selection: string, selectAll: boolean, key: string) => {
    ...
    return (
      ...
      dispatchLocally({
        type: 'select',
        processedSelection: selection === 'null' ? ungroupedSystems : resourceTypes[key]?.[selection],
        key,
      })
    );
  };
