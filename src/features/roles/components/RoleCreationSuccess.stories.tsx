...
export const Default: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    const triggerButton = canvas.getByRole('button', { name: /show role creation success/i });
    await userEvent.click(triggerButton);

    // Test all three action buttons (using localized text)
    const exitButton = canvas.getByRole('button', { name: /exit/i });
    ...
    const createAnotherButton = canvas.getByRole('button', { name: /create another role/i });
    ...
    const addToGroupButton = canvas.getByRole('button', { name: /add role to group/i });
