...
export const RolePermissions: React.FC<RolePermissionsProps> = ({
  ...
  const selection = useDataViewSelection({
    matchOption: (a: any, b: any) => a.uuid === b.uuid,
  });
  ...
  const handleBulkSelect = useCallback(
    (value: BulkSelectValue) => {
      if (value === BulkSelectValue.none) {
        ...
        selection.onSelect(true, paginatedPermissions);
      } else if (value === BulkSelectValue.nonePage) {
        selection.onSelect(false, paginatedPermissions);
      }
    },
    [selection, paginatedPermissions],
  );
  ...
  const bulkSelectComponent = useMemo(() => {
    ...
    const selectedOnPage = tableRows.filter((row) => selection.selected?.some((sel: any) => sel.uuid === row.id)).length;
    ...
    return (
      <ResponsiveActions breakpoint="lg">
        ...
        <ResponsiveAction
          ...
          onClick={() => {
            const multiplePermissionsSelected = selection.selected && selection.selected.length > 1;
            initiateRemove(
              selection.selected || [],
              intl.formatMessage(multiplePermissionsSelected ? messages.removePermissionsQuestion : messages.removePermissionQuestion),
              removeModalText(
                multiplePermissionsSelected ? selection.selected!.length : selection.selected![0].uuid,
                roleName,
  ...
  }, [cantAddPermissions, intl, onNavigateToAddPermissions, selection.selected, initiateRemove, roleName]);
  ...
  return (
    <section className="pf-v5-c-page__main-section rbac-c-role__permissions">
      ...
      {isLoading ? (
        ...
        <DataView selection={!isSystemRole ? selection : undefined}>
          <DataViewToolbar
