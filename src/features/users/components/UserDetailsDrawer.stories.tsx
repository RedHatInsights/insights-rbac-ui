...
export const OpenWithUserDetails: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByTestId('groups-tab-content')).toBeInTheDocument();
    await expect(canvas.getByText('Groups for user: john.doe')).toBeInTheDocument();

    // Should show tab titles
    await expect(canvas.getByText('User groups')).toBeInTheDocument();
    await expect(canvas.getByText('Assigned roles')).toBeInTheDocument();
  },
};
...
export const RolesTabSelected: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // Click on roles tab
    const rolesTab = canvas.getByText('Assigned roles');
    await userEvent.click(rolesTab);

    // Wait for tab switch to complete, then check roles tab content
    await expect(canvas.getByTestId('roles-tab-content')).toBeInTheDocument();
    await expect(canvas.getByText('Roles for user: john.doe')).toBeInTheDocument();
...
export const InteractiveDemo: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const selectButton = canvas.getByText('Select John Doe');
    ...
    // Close drawer
    const closeButton = canvas.getByLabelText('Close drawer panel');
