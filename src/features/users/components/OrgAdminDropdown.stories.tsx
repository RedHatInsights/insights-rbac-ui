...
export const OrgAdminUser: Story = {
  ...
  play: async ({ args, canvasElement }) => {
    ...
    const toggle = canvas.getByTestId('org-admin-dropdown-toggle');
    ...
    const yesButton = canvas.getByRole('menuitem', { name: 'Yes' });
    const noButton = canvas.getByRole('menuitem', { name: 'No' });
    ...
    await waitFor(() => expect(args.onToggle).toHaveBeenCalledWith(false, mockUsername));
  },
  parameters: {
    docs: {
      description: {
...
export const RegularUser: Story = {
  ...
  play: async ({ args, canvasElement }) => {
    ...
    // Verify onToggle was called with correct parameters
    await waitFor(() => expect(args.onToggle).toHaveBeenCalledWith(true, mockUsername));
  },
  parameters: {
    docs: {
      description: {
...
export const InteractiveDemo: Story = {
  ...
  play: async ({ args, canvasElement }) => {
    ...
    const newYesButton = canvas.getByRole('menuitem', { name: 'Yes' });
    await userEvent.click(newYesButton);

    // Verify onToggle was called with correct parameters
    await waitFor(() => expect(args.onToggle).toHaveBeenCalledWith(true, 'demo.user'));
  },
  parameters: {
    docs: {
      description: {
