...
export const FilterRoles: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const filterInput = canvas.getByPlaceholderText(/filter by role/i);
...
export const OrgAdminUser: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(async () => {
      expect(await canvas.findByText(/Org\. Administrator/i)).toBeInTheDocument();
    });

    // For org admin users, a check icon should be present
    const checkIcon = canvasElement.querySelector('.pf-v5-c-icon svg, .fa-check, svg[class*="check"]');
...
export const NonOrgAdminUser: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // For non-org admin users, a close/times icon should be present
    // The icon class is "fa-close" or similar
    const closeIcon = canvasElement.querySelector('.pf-v5-c-icon svg, .fa-times, .fa-close, svg[class*="close"], svg[class*="times"]');
