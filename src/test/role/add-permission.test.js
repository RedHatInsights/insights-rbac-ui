...
const simpleData = [{ permission: 'coolApp:coolResource:read' }];

const duplicateData = [{ permission: 'coolApp:coolResource:read' }, { permission: 'coolApp:coolResource:read' }];

describe('Access compose filter', () => {
  describe('Basic functionality with no filter', () => {
    it('should return all data', () => {
      const out = accessWrapper(simpleData);
      expect(out.data).toEqual([{ application: 'coolApp', resource: 'coolResource', operation: 'read', uuid: 'coolApp:coolResource:read' }]);
    });
  ...
  describe('Compose filter', () => {
    ...
    it('returns all permissions with coolApp and coolResource and readfilter', () => {
      ...
      expect(out.filteredData).toEqual([{ application: 'coolApp', resource: 'coolResource', operation: 'read', uuid: 'coolApp:coolResource:read' }]);
    });
    ...
    it('returns all permissions with (coolApp or differentApp) and (coolResource or firstResource) and read filter', () => {
      const out = accessWrapper(data, {
        applications: ['coolApp', 'differentApp'],
        resources: ['coolResource', 'firstResource'],
        operations: ['read'],
      });
      expect(out.filteredData).toEqual([
        { application: 'coolApp', resource: 'coolResource', operation: 'read', uuid: 'coolApp:coolResource:read' },
  ...
  describe('Not empty', () => {
    ...
    it('Resolve verbs all verb for cool resource', () => {
      const out = resolveSplats(
        [{ uuid: 'app:cool:*' }],
        [
          { application: 'app', resource: 'notCool', operation: 'op' },
          { application: 'app', resource: 'notCool', operation: 'op2' },
          { application: 'app', resource: 'cool', operation: 'op' },
          { application: 'app', resource: 'cool', operation: 'op2' },
        ]
      );
    ...
    it('Resolve all for app', () => {
      const out = resolveSplats(
        ...
        [
          ...
          { application: 'app', resource: 'cool', operation: 'op' },
          { application: 'app', resource: 'cool', operation: 'op2' },
          { application: 'anotherApp', resource: 'cool', operation: 'op' },
          { application: 'anotherApp', resource: 'cool', operation: 'op2' },
        ]
      );
    ...
    it('Resolve all for app and leave other selected be', () => {
      ...
      expect(out).toEqual([
        { uuid: 'app:notCool:op' },
        { uuid: 'app:notCool:op2' },
        { uuid: 'app:cool:op' },
        { uuid: 'app:cool:op2' },
        { uuid: 'anotherApp:superCool:write' },
      ]);
    });
  });
});
