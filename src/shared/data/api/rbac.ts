...
export function createRbacApi(client: AxiosInstance) {
  ...
  return {
    roles: {
      ...
      get: (uuid: string) => client.get<Role>(`/roles/${uuid}/`).then((r) => r.data),

      create: (data: RoleIn) => client.post<Role>('/roles/', data).then((r) => r.data),

      update: (uuid: string, data: Partial<RoleIn>) => client.put<Role>(`/roles/${uuid}/`, data).then((r) => r.data),

      patch: (uuid: string, data: { name?: string; display_name?: string; description?: string }) =>
        client.patch<Role>(`/roles/${uuid}/`, data).then((r) => r.data),

      delete: (uuid: string) => client.delete(`/roles/${uuid}/`),

      getAccess: (uuid: string, params?: { limit?: number; offset?: number }) => client.get(`/roles/${uuid}/access/`, { params }).then((r) => r.data),
    },
    ...
    groups: {
      ...
      get: (uuid: string) => client.get<Group>(`/groups/${uuid}/`).then((r) => r.data),

      create: (data: { name: string; description?: string }) => client.post<Group>('/groups/', data).then((r) => r.data),

      update: (uuid: string, data: { name: string; description?: string }) => client.put<Group>(`/groups/${uuid}/`, data).then((r) => r.data),

      delete: (uuid: string) => client.delete(`/groups/${uuid}/`),
      ...
      getRoles: (uuid: string, params?: { limit?: number; offset?: number }) => client.get(`/groups/${uuid}/roles/`, { params }).then((r) => r.data),

      addRoles: (uuid: string, roleUuids: string[]) => client.post(`/groups/${uuid}/roles/`, { roles: roleUuids }),

      removeRoles: (uuid: string, roleUuids: string[]) => client.delete(`/groups/${uuid}/roles/`, { params: { roles: roleUuids.join(',') } }),
    },

    workspaces: {
      list: (params?: { limit?: number; offset?: number; name?: string; type?: string }) =>
        client
          .get<PaginatedResponse<Workspace>>('/v2/workspaces/', {
            baseURL: v2BaseUrl(),
            params: { type: 'all', ...params },
          })
          .then((r) => r.data),

      get: (id: string) => client.get<Workspace>(`/v2/workspaces/${id}/`, { baseURL: v2BaseUrl() }).then((r) => r.data),
      ...
      delete: (id: string) => client.delete(`/v2/workspaces/${id}/`, { baseURL: v2BaseUrl() }),
