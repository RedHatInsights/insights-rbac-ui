...
import type { ApiError, Group, GroupFilters, Member, SelectedGroupState, ServiceAccount } from './reducer';
...
const selectGroupsListError = (state: RBACStore) => state.groupReducer?.groups?.error; // List errors (ApiError)
const selectGroupError = (state: RBACStore) => state.groupReducer?.error; // Single group errors (e.g., BAD_UUID)
 ...
 */
export const selectGroups = createSelector([selectGroupsState], (groups): Group[] => groups?.data || []);
...
export const selectGroupsPagination = createSelector(
  [selectGroupsState],
  (groups): Partial<PaginationDefaultI> & { redirected?: boolean } => groups?.pagination || groups?.meta || defaultSettings,
);
 ...
 */
export const selectGroupsFilters = createSelector([selectGroupsState], (groups): GroupFilters => groups?.filters || {});

/**
 * Select groups metadata (count, etc)
 * Used in: Pagination, count displays
 */
export const selectGroupsMeta = createSelector([selectGroupsState], (groups) => ({
 ...
 */
export const selectGroupsTotalCount = createSelector([selectGroupsMeta], (meta) => meta.count);
 ...
 */
export const selectGroupExists = createSelector([selectGroupError], (error) => error !== 'BAD_UUID');
 ...
 */
export const selectSystemGroupUUID = createSelector([selectSystemGroupState], (systemGroup): string | undefined => systemGroup?.uuid);
...
export const selectMergedGroupsWithDefaults = createSelector(
  ...
  (groups, adminGroup, systemGroup, filters): Group[] => {
    ...
    const regularGroups = groups.filter((group) => !group.platform_default && !group.admin_default);
...
export const selectGroupsPaginationWithDefaults = (isAdmin: boolean) =>
  createSelector([selectGroupsPagination], (pagination): { limit: number; offset: number; count: number; redirected?: boolean } => {
...
export const selectGroupsListData = createSelector(
  ...
  (groups, pagination, isLoading, totalCount) => ({
    groups,
    pagination,
    isLoading,
    totalCount,
