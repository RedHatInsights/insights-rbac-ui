...
const setRolesForExpandedGroupLoading = (state, { meta }) => ({
  ...
  ...(meta.isPlatformDefault
    ? { systemGroup: { ...state.systemGroup, isLoadingRoles: true } }
    : {
        groups: {
          ...state.groups,
          data: state.groups.data.map((group) => ({
...
const setRolesForExpandedGroup = (state, { payload, meta }) => ({
  ...
  ...(meta.isPlatformDefault
    ? { systemGroup: { ...state.systemGroup, roles: payload.data, isLoadingRoles: true } }
    : {
        groups: {
          ...state.groups,
          data: state.groups.data.map((group) => ({
...
const setMembersForExpandedGroupLoading = (state, { meta }) => ({
  ...state,
  ...(meta.isAdminDefault ? { adminGroup: { ...state.adminGroup, isLoadingMembers: true } } : {}),
  ...(meta.isPlatformDefault
    ? { systemGroup: { ...state.systemGroup, isLoadingMembers: true } }
    : {
        groups: {
          ...state.groups,
          data: state.groups.data.map((group) => ({
