...
export const ModalOpen: Story = {
  play: async ({ canvasElement }) => {
    ...
    expect(within(modal).getByText('Delete group?')).toBeInTheDocument();
...
export const WithWorkspaces: Story = {
  ...
  play: async ({ canvasElement }) => {
    expect(await within(canvasElement).findByText('Workspaces')).toBeInTheDocument();
  },
};
...
export const WithoutWorkspaces: Story = {
  ...
  play: async ({ canvasElement }) => {
    expect(await within(canvasElement).findByText('Legacy view')).toBeInTheDocument();
  },
};
```
