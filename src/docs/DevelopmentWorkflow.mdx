...
When renaming a file with a case change (e.g., `toolbar.js` → `Toolbar.tsx`), you **must** stage both the old deletion and the new file. Otherwise the IDE loses track of the file:

```bash
# ✅ Correct — stage both changes together
git add Toolbar.tsx && git rm toolbar.js
```
