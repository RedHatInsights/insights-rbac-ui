...
webpackFinal: async (config) => {
  config.resolve.alias = {
          ...
          path.resolve(__dirname, '.storybook/hooks/useChrome'),
        '@unleash/proxy-client-react': 
          path.resolve(__dirname, '.storybook/hooks/unleash'),
  };
},
```
...
1. **Update interfaces** in `.storybook/context-providers.tsx`
2. **Update shared types** in `.storybook/types.ts`
...
config.resolve.alias = {
          'new-service-lib': path.resolve(__dirname, '.storybook/hooks/newService'),
};
```
...
- [Storybook Types Reference](./.storybook/types.ts)
- [Context Providers Implementation](./.storybook/context-providers.tsx)
- [Mock Hooks](./.storybook/hooks/)
