...
expect(addMembersSpy).toHaveBeenCalledWith(
  expect.objectContaining({
    ...
    principals: expect.arrayContaining([expect.objectContaining({ username: 'adumble' })]),
  }),
);
```
...
let createdGroupsBad: CreatedGroup[] = [];
let groupMembersBad: Record<string, { users: Principal[]; serviceAccounts: ServiceAccount[] }> = {};
const resetStateBad = () => {
  createdGroupsBad = []; // handlers still see old array
  groupMembersBad = {}; // handlers still see old object
};
```
...
```typescript
import { getUserGroupsTable, findGroupRow, openGroupDrawer, getDrawerScope, verifyUserGroupsTabSelected } from './_shared';
```
...
## No Hardcoded Mock Data Strings

Play functions must reference seed constants — never hardcode entity names, usernames, or workspace names as string literals. When seed data changes, hardcoded strings silently break tests instead of producing a compile error.

Each seed file exports named aliases — import them directly:

```typescript
// ✅ Import named aliases from seed files
import { USER_JOHN, GROUP_SUPPORT_TEAM } from '../../shared/data/mocks/seed';
import { WS_ROOT } from '../../v2/data/mocks/seed';
...
// ❌ Hardcoded strings — break when seed order or names change
...
See `StorybookMandatoryRules.mdx` §11 for the full alias table.
...
| Notification shown | `await expect(within(document.body).findByText(/message/i)).resolves.toBeInTheDocument()` |
