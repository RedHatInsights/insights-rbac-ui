...
- Handle API interactions via React Query
- Coordinate between presentational components
- Use React Query hooks for data fetching and mutations

```jsx
// Example: Feature container with React Query
const Groups = () => {
...
We use **React Query** with feature-based organization:
...
module.exports = {
  moduleFederation: {
    exposes: {
      './CreateWorkspaceWizard': './src/federated-modules/CreateWorkspaceWizard.tsx',
      './WorkspaceSelector': './src/federated-modules/WorkspaceSelector.tsx',
    }
  }
};
```
...
#### 1. Feature Hooks
Encapsulate feature-specific logic using React Query:

```jsx
// src/data/queries/groups.ts
// Query hook - handles fetching and caching automatically
...
// Mutation hook - handles create with cache invalidation
